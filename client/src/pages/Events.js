...
export default class Events extends Component {
  ...
  render() {
    ...
    for(event of this.state.lockins){
      let waiverLink = "";
      let waiverLinkPath = event["waiver_link_path"];
      if(event["waiver_link_path"] != null){
        waiverLink = <a href={waiverLinkPath} target='_blank'>Waiver</a>;
      }
      element = (
        <div key={event["id"]} className='white'>
        <h5 className='title-link' style={{margin: 0}}><a href={'/lockin/info.php?id=' + event["id"]}>{event["title"]}</a></h5>
          <p>{FormattedDate.formatDate(event["event_date"])} {waiverLink}</p>
        </div>  );
